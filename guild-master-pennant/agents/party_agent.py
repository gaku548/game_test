"""
Party Agent - パーティ管理システムの担当エージェント

4人編成と前後列の隊列システムを実装する。
"""
...
class PartyAgent(BaseAgent):
    ...
    def _initialize(self) -> None:
        ...
        初期化処理
        """
        self.logger.info("Initializing Party System...")

        self.update_task_status(
            "party_system",
            "in_progress",
            "Designing 4-member party with formation"
        )

        self.record_decision(
            decision="Implement 2x3 formation with 4 members (3 front, 1 back)",
            rationale="前列3人が60%の被弾、後列1人が10%の被弾でバランスを調整"
        )

        self._initialized = True
        self.send_message("all", "Party system initialized", "info")

    def _generate_gdscript(self) -> None:
        """
        パーティ管理システムのGDScriptを生成
        """
        self.logger.info("Generating Party GDScript...")

        # PartyManager.gd の生成
        party_manager_script = self._create_party_manager_script()
        self.generate_file(
            filepath="game/scripts/PartyManager.gd",
            content=party_manager_script,
            description="パーティ管理 - 4人編成と隊列システム"
        )
        ...
        self.update_task_status(
            "party_system",
            "completed",
            "Party management GDScript generated successfully"
        )

        self.send_message(
            "all",
            "Party system GDScript generated",
            "info",
            {"files": ["PartyManager.gd"]}
        )

    def _create_party_manager_script(self) -> str:
        """
        PartyManager.gd スクリプトを生成
        """
        return '''# PartyManager.gd
# パーティ管理 - 4人編成と隊列システム
    ...
    def _handle_message(self, message: Dict) -> None:
        ...
        if msg_type == "request":
            if "party" in content.lower():
                self.send_message(
                    sender,
                    "Party system is ready. 4-member formation with 3 front + 1 back.",
                    "response"
                )
